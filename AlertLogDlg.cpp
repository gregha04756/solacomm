...
const TCHAR* szAlertLogQuitEvent = {_T("AlertLogQuitEvent")};
...
HANDLE g_hAlertLogQuitEvent;
extern "C++" TCHAR szTitle[];
DWORD WINAPI AlertLogThread(LPVOID lParam);
...
const SOLAALERTDESC AlertDescriptions[] = {
0,_T("None (No alert)"),
...
71,_T("CH 4-20mA signal was invalid"),
72,_T("Flame strength rate differential was invalid"),
73,_T("Flame strength step rate was invalid"),
...
89,_T("Lead Lag all boiler off threshold was invalid"),
...
219,_T("Using backup Lead Lag header sensor due to sensor failure"),
...
225,_T("No anticondensation setpoint was configured for frost protection"),
226,_T("RESERVED"),
227,_T("DHW priority override time was not derated due to invalid outdoor temperature"),
228,_T("Warm weather shutdown was not checked due to invalid outdoor temperature"),
229,_T("Lead Lag slave communication timeout"),
230,_T("RESERVED"),
...
265,_T("No DHW anticondensation setpoint was configured"),
266,_T("No CH anticondensation setpoint was configured"),
...
380,_T("Abnormal Recycle: Fan Speed Not Proven"),
381,_T("Abnormal Recycle: Fan Speed Range Low"),
382,_T("Abnormal Recycle: Fan Speed Range High"),
383,_T("Abnormal Recycle: Pre-Ignition test failed, recycle"),
384,_T("RESERVED"),
385,_T("RESERVED"),
386,_T("RESERVED"),
387,_T("RESERVED"),
388,_T("RESERVED"),
389,_T("Abnormal Recycle: AC power frequency Mismatch"),
390,_T("RESERVED"),
391,_T("RESERVED"),
392,_T("RESERVED"),
393,_T("RESERVED"),
...
446,_T("RESERVED"),
447,_T("RESERVED"),
448,_T("Flame too low"),
449,_T("Modulation rate was limited due to flame strength"),
450,_T("RESERVED"),
...
597,_T("DHW tap detect on threshold was invalid"),
598,_T("DHW plate preheat detect on threshold was invalid"),
599,_T("DHW plate preheat detect off threshold was invalid"),
600,_T("Delta T inlet temperature was invalid"),
601,_T("Delta T outlet temperature was invalid"),
602,_T("Delta T exchanger temperature was invalid"),
603,_T("CH ODR boost max offpoint temperature was invalid"),
604,_T("CH ODR boost max offpoint temperature was too low"),
605,_T("Lead Lag ODR boost max offpoint temperature was invalid"),
606,_T("Lead Lag ODR boost max offpoint temperature was too low"),
607,_T("Mix ODR boost max offpoint temperature was invalid"),
608,_T("Mix ODR boost max offpoint temperature was too low"),
609,_T("Time to rotate lead boiler to next firing slave"),
610,_T("Time to rotate lead boiler to next available slave"),
611,_T("Time to rotate lead boiler to first firing slave in order"),
612,_T("Time to rotate lead boiler to lowest running slave"),
613,_T("Lead boiler was rotated based on new firing sequence order"),
614,_T("Lead boiler was rotated based on measured run time"),
615,_T("Parameter PCB was switched to backup"),
616,_T("Range PCB was switched to backup"),
617,_T("Lead Lag steam pressure was invalid"),
618,_T("Lead Lag steam setpoint was invalid"),
619,_T("Lead Lag steam OFF hysteresis was invalid"),
620,_T("Lead Lag steam ON hysteresis was invalid"),
621,_T("Lead Lag steam minimum pressure was invalid"),
622,_T("Lead Lag modulation sensor was not valid with setpoint source"),
623,_T("Lead Lag modulation sensor was not valid with local setpoint source"),
624,_T("Lead Lag modulation sensor was not valid with local modulation rate source"),
625,_T("Lead Lag steam time of day setpoint was invalid"),
626,_T("Flap failed to close during startup test"),
627,_T("Flap failed to open during startup test"),
628,_T("Flap state did not match fan state"),
629,_T("Disagreement on number of interacting controls"),
630,_T("No interacting controls found in system"),
631,_T("Missing neighbor burner control"),
632,_T("Control was dropped due to communication error"),
633,_T("Incompatible controls detected in system"),
634,_T("Number of controls found did not match the number declared"),
635,_T("Flap valve controls can't be mixed with auxiliary controls"),
636,_T("Paired and single flap valve controls can't be mixed"),
637,_T("Only 2 controls allowed in asymmetrical paired flap valve system"),
638,_T("Asymmetrical paired control requires neighbor to be a paired control"),
639,_T("FVS spare"),
640,_T("Neighbor was not set to monitor for stack faults"),
641,_T("Lead Lag manual modulation test mode selected"),
642,_T("Lead Lag manual modulation test mode was ended"),
643,_T("Lead Lag manual modulation test was suspended due to parameter errors"),
644,_T("Lead Lag manual modulation test was not run - no slave rate was given"),
645,_T("Operation switch was not ON for Lead Lag manual modulation test"),
646,_T("Lead Lag manual modulation test was not run - modulation sensor fault"),
647,_T("Lead Lag manual modulation test was not run - temperature above off setpoint"),
648,_T("Lead Lag manual modulation test was not run - pressure above off setpoint"),
649,_T("DHW demand temperature was invalid"),
650,_T("Stepped modulation rate limiting parameter was invalid"),
651,_T("Hot standby forced rate was invalid"),
652,_T("Hot standby OFF hysteresis was invalid"),
653,_T("Hot standby ON hysteresis was invalid"),
654,_T("Hot standby setpoint was invalid"),
655,_T("Hot standby outlet sensor fault"),
656,_T("Max boiler temp was limited to 140F(60C)-DOE energy saving"),
657,_T("DHW S5 temperature was invalid"),
658,_T("DHW priority time was too short to satisfy the demand"),
659,_T("Modbus analog output was overridden by ART or Modulation output"),
660,_T("RESERVED"),
661,_T("RESERVED"),
662,_T("IAS open step down rate was invalid"),
663,_T("RESERVED"),
664,_T("RESERVED"),
665,_T("RESERVED"),
666,_T("Internal error: EEPROM write verify failed"),
667,_T("Warning: EEPROM writing frequency was too high"),
668,_T("RESERVED"),
669,_T("RESERVED"),
670,_T("RESERVED"),
671,_T("IAS open step down time was invalid"),
672,_T("RESERVED"),
673,_T("RESERVED"),
674,_T("RESERVED"),
675,_T("RESERVED"),
...
677,_T("RESERVED"),
678,_T("RESERVED"),
679,_T("Modbus Heartbeat communication timeout")};

void Update_Alert_List(HWND hwnd_d)
{
	BOOL bResult;
	LRESULT lRes;
	HRESULT hRes;
	int i;
	TCHAR szAlertRecord[MAX_LOADSTRING];
	unsigned short usBit;
	unsigned long ulCycle;

	lRes = ::SendMessage(::GetDlgItem(hwnd_d,IDC_ALERTLIST), LB_RESETCONTENT, (WPARAM) 0, (LPARAM) 0);
			for ( i = 0; !g_bQuit && i < pcAlertLog->GetSize(); i++ )
			{
				if ( pcAlertLog->GetLPMap(i)->pAlertRecord->aa.usAlertCode != 0 )
				{
					usBit = pcAlertLog->GetLPMap(i)->pAlertRecord->aa.usAlertCode;
					ulCycle = pcAlertLog->GetLPMap(i)->pAlertRecord->aa.ulCycle;
					if ( pcAlertLog->GetLPMap(i)->pAlertRecord->aa.usAlertCode < sizeof(AlertDescriptions)/sizeof(SOLAALERTDESC) )
					{
						hRes = ::StringCchPrintf(szAlertRecord, sizeof(szAlertRecord)/sizeof(TCHAR), _T("%03d               %ld             %s"),
							pcAlertLog->GetLPMap(i)->pAlertRecord->aa.usAlertCode,
							pcAlertLog->GetLPMap(i)->pAlertRecord->aa.ulCycle,
							AlertDescriptions[pcAlertLog->GetLPMap(i)->pAlertRecord->aa.usAlertCode].szAlertText);
						lRes = ::SendMessage(::GetDlgItem(hwnd_d,IDC_ALERTLIST), LB_ADDSTRING, (WPARAM) 0, (LPARAM) szAlertRecord);
					}
					else
					{
						hRes = ::StringCchPrintf(szAlertRecord, sizeof(szAlertRecord)/sizeof(TCHAR), _T("%03d               %ld             Invalid Alert Code"),
							pcAlertLog->GetLPMap(i)->pAlertRecord->aa.usAlertCode,
							pcAlertLog->GetLPMap(i)->pAlertRecord->aa.ulCycle);
						lRes = ::SendMessage(::GetDlgItem(hwnd_d,IDC_ALERTLIST), LB_ADDSTRING, (WPARAM) 0, (LPARAM) szAlertRecord);
					}
			...
			if (2 != pcBurnerControlStatus->GetValue((unsigned short)0x0023))
			{
				bResult = SetDlgItemText(hwnd_d,IDC_LBLALARMALERT,_T(""));
				bResult = SetDlgItemText(hwnd_d,IDC_TXTALARMALERT,_T(""));
			}
			if (2 == pcBurnerControlStatus->GetValue((unsigned short)0x0023))
			{
				bResult = SetDlgItemText(hwnd_d,IDC_LBLALARMALERT,_T("Alert causing alarm:"));
				if (0 != pcAlarmCode->GetValue((unsigned short)0x045f))
				{
					if (pcAlarmCode->GetValue((unsigned short)0x045f) < sizeof(AlertDescriptions)/sizeof(SOLAALERTDESC))
					{
						hRes = ::StringCchPrintf(szAlertRecord, sizeof(szAlertRecord)/sizeof(TCHAR), _T("%03d %s"),
							pcAlarmCode->GetValue((unsigned short)0x045f),
							AlertDescriptions[pcAlarmCode->GetValue((unsigned short)0x045f)].szAlertText);
						lRes = SetDlgItemText(hwnd_d,IDC_TXTALARMALERT,szAlertRecord);
					}
				}
			}
}

LRESULT CALLBACK AlertLogDlgProc(HWND hDlg, UINT uMessage, WPARAM wParam, LPARAM lParam)
{
	LRESULT lRes;
	HRESULT hRes;
	...
	unsigned short usBit;
	DWORD dwResult;
	static DWORD dwAlertLogThreadID;
	LPSUMMARYTHREADPARMS pSummaryParms;
	static HANDLE hAlertLogThread;
	static HANDLE hAlertLogThreadDup;
	TCHAR szAlertRecord[MAX_LOADSTRING];
	...
	case WM_INITDIALOG:
		...
		nPageIndex = (int) ::SendMessage(::GetParent(hDlg), PSM_IDTOINDEX, 0, (LPARAM)IDD_ALERTLOGDLG);
		if ( bSuccess )
		{
			// Create Summary Quit event
			g_hAlertLogQuitEvent = ::CreateEvent(NULL, true, false, szAlertLogQuitEvent);
			bResult = ::ResetEvent(g_hAlertLogQuitEvent);
			// Start Alert Log thread
			...
			pSummaryParms->hQuitEvent = g_hAlertLogQuitEvent;
			pSummaryParms->nPg = nPageIndex;
			hAlertLogThread = ::CreateThread(NULL, 0, AlertLogThread, (LPVOID) pSummaryParms, CREATE_SUSPENDED, &dwAlertLogThreadID);
			bResult = ::DuplicateHandle(::GetCurrentProcess(), hAlertLogThread, GetCurrentProcess(), &hAlertLogThreadDup, 0, false, DUPLICATE_SAME_ACCESS);
			dwResult = ::ResumeThread(hAlertLogThread);
		}
		usBit = 1;
...
//		lpnmhdr = (LPNMHDR)&(lppsn->hdr);
		...
		case PSN_APPLY:   //sent when OK or Apply button pressed
			if ( lppsn->lParam )	// lParam TRUE if OK button
			{
				if ( hAlertLogThread )
				{
					bResult = ::SetEvent(g_hAlertLogQuitEvent);
					dwResult = ::WaitForSingleObject(hAlertLogThread, 5000);
					switch (dwResult)
					{
					case WAIT_TIMEOUT:
						::MessageBox( hDlg, _T("Wait for Alert Log update thread completion timed out"), szTitle, MB_OK );
						break;
					case WAIT_OBJECT_0:
						break;
					}
					bResult = ::CloseHandle(hAlertLogThreadDup);
					bResult = ::CloseHandle(hAlertLogThread);
					bResult = ::CloseHandle(g_hAlertLogQuitEvent);
				}
			}
			break;
...
DWORD WINAPI AlertLogThread(LPVOID lpParam)
{
	BOOL bResult;
	BOOL bSuccess = true;
	int nPage;
	...
	bResult = ::DuplicateHandle(::GetCurrentProcess(), g_hAlertLogQuitEvent, GetCurrentProcess(), &hEvents[0], 0, false, DUPLICATE_SAME_ACCESS);
	...
	while ( !g_bQuit && bSuccess && ((dwResult = ::WaitForSingleObject(hEvents[0], 0))==WAIT_TIMEOUT) )
	{
		...
		{
			bResult = ::PostMessage(hParentWnd, WM_APPALERTUPD, (WPARAM) 0, (LPARAM) 0);
			::EnterCriticalSection(&g_UpdCountCS);
			nUpdCount = (bResult) ? ++nUpdCount : nUpdCount;
			::LeaveCriticalSection(&g_UpdCountCS);
			if ( !bResult )
			{
				::MessageBox(hParentWnd, _T("Alert thread PostMessage error"), szTitle, MB_OK);
			}
		}
