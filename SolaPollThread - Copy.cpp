				...
				case LPInitIDreq:
					...
					for ( ndx = 0; ndx < lpMap->GetRegGroupSize(); ndx++ )
					{
						...
						case Novalue:
							break;
						case Stringvalue:
							if (lpMap->GetType((int)ndx) == Stringvalue && (lpMap->GetStr((int)ndx) != NULL))
							{
...
DWORD WINAPI CSolaTCPComm::SolaPollThread(LPARAM lParam)
{
	...
	bResult = ::DuplicateHandle(::GetCurrentProcess(),pThis->m_hQuitEvent,::GetCurrentProcess(),&hEvents[0],DUPLICATE_SAME_ACCESS,true,DUPLICATE_SAME_ACCESS);
	bResult = ::DuplicateHandle(::GetCurrentProcess(),pThis->m_hResponseEvent,::GetCurrentProcess(),&hEvents[1],DUPLICATE_SAME_ACCESS,true,DUPLICATE_SAME_ACCESS);
	bResult = ::DuplicateHandle(::GetCurrentProcess(),pThis->m_hQuitEvent,::GetCurrentProcess(),&hTimerEvents[0],DUPLICATE_SAME_ACCESS,true,DUPLICATE_SAME_ACCESS);
	...
	bResult = ::DuplicateHandle(::GetCurrentProcess(),pThis->m_hQuitEvent,GetCurrentProcess(),&hPageEvents[0],0,false,DUPLICATE_SAME_ACCESS);
...
/* Main loop */
	while ( !pThis->m_bQuit && bSuccess )
	{
		if (NULL != pThis->m_lpSolaID)
		{
			delete pThis->m_lpSolaID;
			pThis->m_lpSolaID = NULL;
		}
		if (NULL != pThis->m_szOSNumber)
		{
			delete[] pThis->m_szOSNumber;
			pThis->m_szOSNumber = NULL;
		}
		if (NULL != pThis->m_szBurnerName)
		{
			delete[] pThis->m_szBurnerName;
			pThis->m_szBurnerName = NULL;
		}
		if (NULL != pThis->m_szOEMID)
		{
			delete[] pThis->m_szOEMID;
			pThis->m_szOEMID = NULL;
		}
		while ((TCP_Protonode != pThis->Get_Gateway_Type()) && !pThis->m_bQuit && !pThis->IsSolaConnected())
		{
			...
			lpmbm->mbaphdr.uchUnitIdentifier = SOLAMBAddress;
			...
			nSendResult = send(pThis->m_MBSocket,(char*)&(lpmbm->mbaphdr),sizeof(MBAPHEADER)+sizeof(MBREQUEST),0);
			...
			case WAIT_OBJECT_0 + 1:
				...
				bResult = pThis->ProcessResponse(pThis);
				if ( !pThis->IsSolaConnected() )
				{
					dwWaitResult = ::WaitForMultipleObjects(sizeof(hEvents)/sizeof(HANDLE),hEvents,false,5000);
				}
				break;
		...
		} // End of while ( !TCPPROTONODE && !pThis->m_bQuit && !pThis->IsSolaConnected() )
		
		while ( (TCP_Protonode == pThis->Get_Gateway_Type()) && !pThis->m_bQuit && !pThis->IsSolaConnected() )
		{
			dwResult = ReadSolaMap(
				(CSolaTCPComm*)pThis,
				(unsigned char const)SOLAMBAddress,
				pc_Dup_Burner_Name,
				lpmbm,
				LPInitIDreq,
				hEvents,
		...
		if ( !pThis->m_bQuit && bReConnect && pThis->IsSolaConnected() )
		{
			bReConnect = false;
			bResult = ::PostMessage(pThis->m_hwndParent, WM_APPDATAUPDSTART, (WPARAM) 0, (LPARAM) 0);
			pnd = (CNoticeDialog*)new CNoticeDialog(pThis->m_hwndParent,g_hInst,szTitle);
...
/* Get lockout log */
			dwResult = Read_Lockout_Log(
				...
				bReConnect,
				pnd);
			bResult = ::PostMessage(pThis->m_hwndParent, WM_APPREADOK, (WPARAM) 0, (LPARAM) 0);
			pnd->Stop();
			delete pnd;
			pnd = NULL;
		}

		while ( !pThis->m_bQuit && pThis->IsSolaConnected() && bSuccess )
		{
...
// Check if there's a status update
			bStatusChange = false;
			usBit = 1;
			for ( i = 0; !pThis->m_bQuit && i < pcStatusChangeCodes->GetSize(); i++ )
			{
			...
			for ( i = 0; !pThis->m_bQuit && i < pcConfigChangeCodes->GetSize() && pcSystemStatus->GetLPMap(1)->sValue; i++ )
			{
			...
			} /* End if (TCP_Protonode != pThis->Get_Gateway_Type()) */
...
/* Only get register groups needed for the currently displayed page */
				if (bSuccess && !g_bQuit && pThis->IsSolaConnected())
				{
					EnterCriticalSection(&gRWDataCritSect);
...
/* Get Lockout history log */
			...
			::LeaveCriticalSection(&gRWDataCritSect);
			while ( !pThis->m_bQuit && !bMBSndRcvQmt && pThis->IsSolaConnected() ) /* Check for data to be sent to Sola */
			{
				i = 0;
				::SecureZeroMemory((PVOID)MBMsg,(SIZE_T)sizeof(MBMsg));
				::EnterCriticalSection(&gRWDataCritSect);
				while ( !pThis->m_bQuit && *g_MBSndRcvReqQ.front().ppchToSnd < *g_MBSndRcvReqQ.front().ppchEndSnd && *g_MBSndRcvReqQ.front().ppchToSnd < g_MBSndRcvReqQ.front().pchSndBuf + g_MBSndRcvReqQ.front().nSndBufSize  )
				{
				...
				case 0x10: /* Write Multiple Registers */
					...
					delete[] lpMBMsg;
					nSendResult = send(pThis->m_MBSocket,(char*)MBMsg,cbLen,0);
					if ( nSendResult == SOCKET_ERROR )
					{
						hRes = ::StringCchPrintf(szError,sizeof(szError)/sizeof(TCHAR),_T("send() failed with error %d"),::WSAGetLastError());
						nResult = ::MessageBox(pThis->m_hwndParent,szError,szTitle,MB_OK);
		...
		} /* End of 1st inner loop while ( !pThis->m_bQuit && pThis->IsSolaConnected() && bSuccess ) */
	
	} /* End of main loop while ( !pThis->m_bQuit && bSuccess ) */
	...
	{
		pThis->Set_Sola_Connected(false);
		bResult = ::PostMessage(pThis->m_hwndParent,WM_APPSOLAPOLLABORT,(WPARAM)0,(LPARAM)0);
	}
	if ( h1SecTimer )
	{
